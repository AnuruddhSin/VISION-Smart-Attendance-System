...
def faculty_dashboard():
    if 'facultyId' in session:
        ...
        mycursor.execute("SELECT DISTINCT Branch, subject FROM timetable WHERE facultyId = %s", (facultyId,))
        timetable_data = mycursor.fetchall()

        mycursor.execute("""
                    SELECT DISTINCT s.fullName, s.StudentID, t.Branch, t.subject 
                    FROM student_data s
        ...
        for student in student_data:
            ...
            mycursor.execute("""
                            ...
                            WHERE StudentID = %s AND Branch = %s AND subject=%s
                        """, (student_id, branch, subject))
            percentage = mycursor.fetchone()[0]
            student_percentage[student_id] = percentage or 'N/A'
